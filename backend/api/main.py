...
def create_heatmap_overlay(heatmap, original_img_bytes, threshold=0.35):
    ...
    try:
        ...
        alpha = 0.4
        blended_region = cv2.addWeighted(img_array, 1 - alpha, heatmap_colored, alpha, 0)
...
async def chat_endpoint(request: ChatRequest = Body(...), user: dict = Depends(get_current_user)):
    ...
    if request.context:
        try:
            ...
            medical_context = format_context_for_prompt(condition_info, diagnosis_label, confidence)
            
            print(f"📋 Context parsed - Diagnosis: {diagnosis_label}, Confidence: {confidence}%")
...
"""
    ...
    print(f"💬 Processing chat - Message: '{request.message[:50]}...' with {len(request.history)} history items")
